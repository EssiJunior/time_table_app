...
def create_a_filiere(filiere: schemas.FiliereCreate, db: Session = Depends(get_db),
    ...
    if isinstance(current_user, models.Administrateur):
        ...
        #db.add(filiere)
        requete_SQL = insert(models.Filiere).values(**filiere.dict())
        db.execute(requete_SQL)      #db.commit()
        #db.refresh(filiere)
        return {"code": filiere.code, "nom":filiere.nom,
                "created_at": datetime.now(), "requete_SQL": requete_SQL}
    else:
        raise HTTPException(status_code = status.HTTP_401_UNAUTHORIZED, detail=f"Désolé, seul un administrateur peut realiser cette tache.")


@router.get("/all", response_model= List[schemas.FiliereResponse])
def display_all_filieres(db: Session = Depends(get_db)):  
    requete_SQL = db.query(models.Filiere)
    filieres = requete_SQL.all()
    all_together = filieres.append(requete_SQL)
    return all_together
...
def display_a_specific_filiere(code: str, db: Session = Depends(get_db),
    ...
    print("Current User: ",type(current_user))
    if isinstance(current_user, models.Administrateur):
        requete_SQL = db.query(models.Filiere).filter(models.Filiere.code == code)
        filiere = requete_SQL.first()
        if not filiere:
            raise HTTPException(status_code = status.HTTP_404_NOT_FOUND, detail=f"La Filiere codée << {code} >> n'existe pas ")
        return {"code": filiere.code, "nom":filiere.nom, "requete_SQL": requete_SQL}
    else:
        raise HTTPException(status_code = status.HTTP_401_UNAUTHORIZED, detail=f"Désolé, seul un administrateur peut realiser cette tache.")
...
def delete_a_filiere(code: str, db: Session = Depends(get_db),
    ...
    if isinstance(current_user, models.Administrateur):
        ...
        if filiere.first() == None:
            ...
            requete_SQL = filiere.delete(synchronize_session = False)
            db.commit()
...
def update_a_filiere(code: str, filiere: schemas.FiliereCreate, db: Session = Depends(get_db),
    ...
    if isinstance(current_user, models.Administrateur):
        ...
        requete_SQL = response.update(filiere.dict(),synchronize_session=False)
        db.commit()
        return {"code": filiere.code, "nom":filiere.nom,
                "created_at": datetime.now(), "requete_SQL": requete_SQL}
    else:
